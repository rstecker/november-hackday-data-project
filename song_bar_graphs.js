...
if (true) {
  ...
  nv.addGraph(function() {
    ...
    graph_data = unmodified_values(all_plays);
...
function play_chooser(p) {
  if (trackIndex == 4)
    return p[2] - p[3]
  return p[trackIndex];
}
function all_plays(data_entry) {
  var val =  _.reduce(data_entry.plays, function(memo, p){
    if (_.contains(date_list, p[0]))
      return memo + play_chooser(p);
    return memo;
  }, 0)
  return bucketer(val);
}
  ...
  // iPad = 7
  // Xbox = 8
function web_only_plays(data_entry) {
  var val = _.reduce(data_entry.plays, function(memo, p){
    if (p[1] == 0)
      return memo + play_chooser(p);
    return memo;
...
function iphone_only_plays(data_entry) {
  var val =  _.reduce(data_entry.plays, function(memo, p){
    if (p[1] == 2 && _.contains(date_list, p[0]))
      return memo + play_chooser(p);
    return memo;
  }, 0)
  return bucketer(val);
}
function android_only_plays(data_entry) {
  var val =  _.reduce(data_entry.plays, function(memo, p){
    if (p[1] == 3 && _.contains(date_list, p[0]))
      return memo + play_chooser(p);
    return memo;
  }, 0)
  return bucketer(val);
}
function other_only_plays(data_entry) {
  var val =  _.reduce(data_entry.plays, function(memo, p){
    if ((p[1] == 1 || p[1] > 3)  && _.contains(date_list, p[0]))
      return memo + play_chooser(p);
    return memo;
  }, 0)
  return bucketer(val);
